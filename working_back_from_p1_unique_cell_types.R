...
PPN1.ids <- cf_meta(cf_ids(malecns='AN05B102')) %>% pull(id)

#find subgroup of P1 outputs of PPN1 and vAB3
PPN1.unique.output.P1.id <- cf_partners(cf_ids(malecns=PPN1.ids), partners = 'out',threshold=5) %>% 
  filter(grepl("^P1_([0-9]|[1-9][0-9])[a-zA-Z]$", type)) %>%
  pull(post_id) %>% 
  unique()

vAB3.unique.output.P1.id <- cf_partners(cf_ids(malecns=vAB3.ids), partners = 'out',threshold=5) %>% 
  filter(grepl("^P1_([0-9]|[1-9][0-9])[a-zA-Z]$", type)) %>%
  pull(post_id) %>% 
  unique()
...
vAB3.output.mAL.unique.output.P1.id <- cf_partners(cf_ids(malecns=vAB3.output.mAL.id), partners = 'out',threshold=5) %>% 
  filter(grepl("^P1_([0-9]|[1-9][0-9])[a-zA-Z]$", type)) %>%
  pull(post_id) %>% 
  unique() 
...
venn.plot <- venn.diagram(
  ...
  main = "Venn Diagram of the P1 neurons (per neuron, not type) the contact chemonsensation is convering onto" # Title
)
...
DA1.out.P1.outout.unique.mAL.id <- cf_partners(cf_ids(malecns=DA1.unique.output.P1.id), partners = 'out',threshold=5) %>%
  filter(grepl("^mAL_(([a-zA-Z]([0-9]|[1-9][0-9])[a-zA-Z]?))$", type)) %>%
  pull(post_id) %>% 
  unique()


a = vAB3.output.mAL.id
b = DA1.out.P1.outout.unique.mAL.id
...
AN05B035.out.unique.mAL.type <- cf_partners(cf_ids(malecns = 'AN05B035'), partners = 'out',threshold=5) %>%
  ...
  pull(type) %>% 
  unique()
AN05B035.out.unique.mAL.id <- cf_partners(cf_ids(malecns = 'AN05B035'), partners = 'out',threshold=5) %>%
  filter(grepl("^mAL_(([a-zA-Z]([0-9]|[1-9][0-9])[a-zA-Z]?))$", type)) %>%
  pull(post_id) %>% 
  unique()
...
a = vAB3.sensory.input.unique.id
b = PPN1.sensory.input.unique.id

dev.off()
venn.plot <- venn.diagram(
  x = list(vAB3.input.cells=a, PPN1.input.cells=b),  # Define the three sets
  filename = NULL,                 # No file output
  fill = c("#FFC145", "#5B5F97"), # Colors for the circles
  alpha = 0.5,                     # Transparency of the circles
  cex = 2,                         # Text size inside the circles
