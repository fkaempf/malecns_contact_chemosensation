...
active_neuron_cols <- colSums(activation_matrix) > 0
activation_matrix_filtered <- activation_matrix[, active_neuron_cols, drop = FALSE]
pheatmap(cor(activation_matrix_filtered, pattern_activity_matrix))
```
cluster
```{r}
...
```{r}
# Run PCA
pca_res <- prcomp(pattern_activity_matrix_scaled, center = TRUE, scale. = TRUE)
