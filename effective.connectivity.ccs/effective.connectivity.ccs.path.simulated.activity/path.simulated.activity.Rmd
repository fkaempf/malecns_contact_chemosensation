...
target_types <- c(SAG,P1,TK,pCd,pMP2,pIP10,CL062)
```
...
distance_to_start <- shortest.paths(g.sub.prout.mcns, to = V(g.sub.prout.mcns)[name %in% start_types], weights = NA)
...
code function
```{r}
simulate <- function(g, start,epochs=100,stim_epochs=5){
...
series <- as.matrix(t0[ , setdiff(names(t0), "time")])
axis(1, at = 1:nrow(series), labels = 1:nrow(series))
```
